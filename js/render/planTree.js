...
// ── Link / children helpers ─────────────────────────────────────────────────
// A node consumes from at most one pipeline-root "peer": a cross-fragment
// DATA_STREAM_SINK (EXCHANGE side) or a within-fragment pipeline sink (source
// side). Both are laid out as a child hanging below the consumer, so the whole
// query renders as one connected tree spanning fragment and pipeline breaks.

export function linkPeer(node) {
  if (node.crossFragmentLink && node.crossFragmentLink.peerIdx !== null) {
    return node.crossFragmentLink.peerIdx;
  }
  if (node.pipelineLink && node.pipelineLink.peerIdx !== null) {
    return node.pipelineLink.peerIdx;
  }
  return null;
}

// Effective layout children: the operator children within the pipeline plus the
// linked peer (if any). The peer is appended last so a join's build side (linked
// sink) sits to the right of its probe side (operator child).
export function childrenForLayout(plan, idx) {
  const node = plan.nodes[idx];
  const kids = node.childrenIdx.slice();
  const peer = linkPeer(node);
  if (peer !== null) kids.push(peer);
  return kids;
}
...
function bfsFrom(plan, depths, startIdx, startDepth) {
  ...
  while (queue.length) {
    ...
    for (const childIdx of childrenForLayout(plan, idx)) {
...
// BFS from rootIdx over the unified link tree (operator children + link peers,
// see childrenForLayout), so each linked sink lands one level below the operator
// that consumes it. After the main BFS, any pipeline root still at depth=null
// (a disconnected pipeline / unmatched sink) is anchored at the minimum depth of
// its fragment (or 0), then BFS'd from there.
...
export function computeSubtreeWidths(plan) {
  ...
  const seen = new Set();   // guards against malformed cyclic link data
  function recur(idx) {
    if (seen.has(idx)) return widths[idx] || NODE_W;
    seen.add(idx);
    const kids = childrenForLayout(plan, idx);
    if (kids.length === 0) {
      widths[idx] = NODE_W;
      return NODE_W;
    }
...
// Pre-order x-assignment over the unified link tree rooted at rootIdx. Because
// cross-fragment (EXCHANGE→DATA_STREAM_SINK) and within-fragment (source→sink)
// links are laid out as children (see childrenForLayout), the root's subtree
// already spans every connected fragment and pipeline — each peer is centered
// directly below the operator that consumes it. Any genuinely disconnected
// pipeline root (e.g. an unmatched sink) is placed to the right afterward.
export function layoutPlan(plan) {
  ...
  function assignSubtree(idx, centerX) {
    if (placed.has(idx)) return;   // guards against a peer reached twice
    placed.add(idx);
    pos[idx].x = centerX;
  ...
  // Place a subtree at a preferred center, shifting right to clear any range it
  // would overlap, then record its occupied horizontal extent.
  const placedRanges = [];
  function placeComponent(idx, preferredCenterX) {
    const w = widths[idx];
    ...
    placedRanges.push({ xMin, xMax });
  }

  if (plan.rootIdx !== null) placeComponent(plan.rootIdx, 0);

  // Place any remaining pipeline roots not reached through the root's link tree
  // (disconnected pipelines / unmatched sinks) to the right of everything placed.
  for (const { idx } of plan.pipelineRoots) {
    if (placed.has(idx)) continue;
    const rightmost = placedRanges.length > 0
      ? placedRanges.reduce((max, r) => (r.xMax > max ? r.xMax : max), -Infinity)
      : -widths[idx] / 2 - H_GAP;
    placeComponent(idx, rightmost + H_GAP + widths[idx] / 2);
  }
...
function renderNodesAndEdges(plan, nodesG, edges) {
  ...
  for (const node of plan.nodes) {
    if (!node.pipelineLink || node.pipelineLink.peerIdx === null) continue;
    edges.appendChild(svgEl('path', {
      class: 'edge pipeline',
      d: edgePath(pos[node.pipelineLink.peerIdx], pos[node.idx]),
    }));
  }
...
function renderPanelContent(plan, idx, panel) {
  ...
  if (node.pipelineLink && node.pipelineLink.peerIdx !== null) {
    const peer = plan.nodes[node.pipelineLink.peerIdx];
    panel.appendChild(el('h3', {}, 'Pipeline link'));
    panel.appendChild(el('div', { class: 'xfrag-info' }, [
      el('div', {}, `↓ reads from ${peer.name} (id=${node.pipelineLink.linkId})`),
      el('div', {}, 'pipeline breaker in the same fragment'),
      el('button', {
        class: 'reveal-btn',
        'data-peer': String(peer.idx),
      }, 'Reveal in tree'),
